...
def shortest_string(strings):
    ...
    for k in range(len(strings)):
        if len(strings[k]) < len(strings[shortest_string]):
            shortest_string = k
    return strings[shortest_string]
...
def is_palindrome(s):
    ...
    ####################################################################
    # ------------------------------------------------------------------
    end_to_middle = len(s) - 1
    for k in range(len(s)//2):
        if s[k] == s[end_to_middle]:
            end_to_middle = end_to_middle - 1
    if s[end_to_middle] == s[len(s)//2]:
        return True
    return False
